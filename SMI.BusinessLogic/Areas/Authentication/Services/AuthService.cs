...
using SMI.Common.API.Providers;
...
namespace SMI.BusinessLogic.Areas.Authentication.Services
{
    ...
    { 
        ...
        private readonly IOptions<BaseClient> _baseClientConfig;
        public AuthService(
            IHttpClientFactory httpClientFactory, 
            IOptions<BaseClient> baseClientConfig)
        {
            _httpClientFactory = httpClientFactory;
            _baseClientConfig = baseClientConfig;
        }
        /// <summary>
        /// SocialMedia SignIn
        /// </summary>
        ...
        public async Task<JwtResponseVM> SignInWithSocialMedia(SocialMediaVM model)
        {
            if (model != null)
            {
                if (model.LoginProvider == LoginProvider.Facebook)
                {
                    //var fbClient =await GetFacebookClient();

                   var fbClient = _httpClientFactory.CreateClient(ProviderType.FacebookClient);
                    fbClient.BaseAddress = _baseClientConfig.Value.BaseUrl;
